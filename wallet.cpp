...
// WARNING: private key is stored as plaintext hex — do not share this file.
// Future work: encrypt with a KDF (e.g. Argon2 + AES-256-GCM) before storing.

static std::string toHexW(const unsigned char* data, size_t len) {
    std::stringstream ss;
...
void Wallet::saveToFile(const std::string& filename) const {
    ...
    file << privateKeyHex << "\n" << publicKeyHex << "\n";
    file.close();
    // Restrict to owner read/write only — private key must stay secret.
